...
//  Nga Pham
//
//  Created by Tuan Dang Q. on 3/28/20.
...
class SecurePreference {
    ...
    func set<Key: RawRepresentable>(_ value: Date, forKey key: Key) where Key.RawValue == String {
        userDefaults.set(value, forKey: key.rawValue)
    }
    ...
    func date<Key: RawRepresentable>(forKey key: Key) -> Date? where Key.RawValue == String {
        guard let value = userDefaults.object(forKey: key.rawValue) else { return nil }
