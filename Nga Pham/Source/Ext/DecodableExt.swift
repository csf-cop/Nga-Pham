...
extension Decoder {
    ...
    func decode<K: CodingKey, F: AnyDateFormatter>(_ key: K, using formatter: F) throws -> [Date] {
        let container = try self.container(keyedBy: K.self)
        let rawStrings: [String] = try container.decode([String].self, forKey: key)
        var dates: [Date] = []
    ...
    func decodeStringIfPresent(_ key: String) throws -> String? {
        return try decodeIfPresent(AnyCodingKey(key), as: String.self)
    }
}
